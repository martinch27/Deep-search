...
class IterativeResearcher:
    ...
    async def run(
            ...
            background_context: str = ""   # <-- теперь принимаем и этот параметр
        ) -> str:
        """Run the deep research workflow for a given query."""
        self.start_time = time.time()

        # 2) Извлекаем факты из текущего запроса и дополняем сессию
        fact_agent = init_fact_extractor_agent(self.config)
        fact_res = await ResearchRunner.run(fact_agent, query)
        extracted = fact_res.final_output_as(FlexibleExtractionOutput)
        # pydantic-модели -> обычные dict
        new_items = [item.dict() for item in extracted.extracted]
        self.session_manager.append_extracted_items(new_items)
        # 3) Загружаем всё, что есть в SessionMemory по текущему session_id
        stored = self.session_manager.load_extracted_items()

        # 3) Собираем весь накопленный background-контекст для этой сессии
        ...
        # (далее по коду мы будем передавать background_context в каждый шаг)

        workflow_trace = self.tracing if self.tracing else None
        if workflow_trace:
    ...
    async def _execute_tools(self, tasks: List[AgentTask]) -> Dict[str, ToolAgentOutput]:
        ...
        with custom_span("Execute Tool Agents"):
            ...
            critique_prompt = (
                f"ORIGINAL QUERY:\n{self.conversation.get_latest_gap()}\n\n"
                f"FINDINGS FROM CURRENT ITERATION:\n{chr(10).join(findings)}\n\n"
                f"ALL FINDINGS SO FAR:\n{chr(10).join(self.conversation.get_all_findings())}"
            )

            critique_result = await ResearchRunner.run(self.critic_agent, critique_prompt)
            critique = critique_result.final_output_as(CritiqueOutput)

            self._log_message(f"<critique>\n{critique.critique_summary}\n</critique>")
            if critique.follow_up_question:
                self._log_message(f"<follow_up>\nSuggested follow-up: {critique.follow_up_question}\n</follow_up>")
    ...
    async def _generate_observations(self, query: str, background_context: str = "") -> str:
        ...
        CURRENT FINDINGS:
        {self.conversation.get_all_findings()}
